...
def build_bq_schema_from_descriptor(descriptor: monitoring_v3.TimeSeriesDescriptor):
  schema = [
      bigquery.SchemaField("start_time", "TIMESTAMP"),
      bigquery.SchemaField("end_time", "TIMESTAMP", mode="REQUIRED"),
  ]
  for label in descriptor.label_descriptors:
    label_value_type=_LABELDESCRIPTOR_VALUETYPE.values_by_number[label.value_type].name
    key=re.sub('[^a-zA-Z0-9_]','_', label.key)
    schema.append(bigquery.SchemaField(key, label_value_type, description = label.description))
  for point in descriptor.point_descriptors:
    kind=_METRICDESCRIPTOR_METRICKIND.values_by_number[point.metric_kind].name
    point_type=_METRICDESCRIPTOR_VALUETYPE.values_by_number[point.value_type].name
    point_key=re.sub('[^a-zA-Z0-9_]','_', point.key)
    if point_type == "DOUBLE":
      point_type = "FLOAT"
    schema.append(bigquery.SchemaField(point_key, point_type, description=f"{kind} metric, {point.unit}"))
  return schema
...
def create_bq_table(export_project, dataset, table_name, schema, client: bigquery.Client):
  table_id=f"{export_project}.{dataset}.{table_name}"
  table=bigquery.Table(table_id, schema=schema)
  table.time_partitioning=bigquery.TimePartitioning(type_="DAY", expiration_ms=(1000*60*60*24*90), field='end_time')
  try:
    t=client.create_table(table)
    logging.info("Created table {}.{}.{}".format(t.project, t.dataset_id, t.table_id))
  except (AlreadyExists, Conflict):
    logging.debug("Table {}.{}.{} already exists".format(table.project, table.dataset_id, table.table_id))
  return bigquery.Table(table_id, schema=schema)

def new_export_metric(export_project, dataset, table, query, project, client: monitoring_v3.QueryServiceClient, bq_client: bigquery.Client):
  ...
  if not check_if_exists(table_id, bq_client):
    table_obj=create_bq_table(export_project, dataset, table, schema, bq_client)

  for data in pager:
    entry={}
    rows=[]
    for label_val, label_descript in zip(data.label_values, descriptor.label_descriptors):
      val_type=_LABELDESCRIPTOR_VALUETYPE.values_by_number[label_descript.value_type].name
    ...
    for point in data.point_data:
      ...
      for col_value, col_descript in zip(point.values, descriptor.point_descriptors):
        col_type=_METRICDESCRIPTOR_VALUETYPE.values_by_number[col_descript.value_type].name
  ...
  with ThreadPoolExecutor(max_workers=10) as executor:
    ...
    for project in projects:
      ...
      for metric, query in queries.items():
        ...
        futures.append(executor.submit(new_export_metric, config.PROJECT_ID, config.BIGQUERY_DATASET, metric_table_name, query, project, monitoring_client, bq_client))
      for fut in as_completed(futures):
        try:
          total_rows+=fut.result()
        except Exception as err:
          failures.append(err)
      logging.info(f"Finished exporting {total_rows} data points for project {project}")
      if failures:
        logging.error(f"Failures occured while processing {len(failures)}/{len(queries)*len(projects)} queries!")
